...
class PPOAgent(object):
        ...
        for _ in range(self.n_epochs):
            ...
            for batch in batches:
                with tf.GradientTape(persistent=True) as tape:
                    boolean_map_in = state_bool_arr.get_boolean_map[batch][tf.newaxis, ...] #TODO: Check if not changing into tensorflow works
                    float_map_in = state_float_arr.get_float_map[batch][tf.newaxis, ...]
                    scalars = np.array(state_scaler.get_scalars[batch], dtype=np.single)[tf.newaxis, ...]
                    ...
                    critic_value = self.value_output()
    ...
    def value_output(self, state):

        boolean_map_in = state.get_boolean_map()[tf.newaxis, ...]
        float_map_in = state.get_float_map()[tf.newaxis, ...]
        scalars = np.array(state.get_scalars(), dtype=np.single)[tf.newaxis, ...]

        return self.get_value_output([boolean_map_in, float_map_in, scalars]).numpy()[0]
